...
void from_json(const json& j, Opt<Device>& device)
{
    ...
    if (std::find(DeviceMan::SKIP_CAP.begin(), DeviceMan::SKIP_CAP.end(), GetType(type).value_or(DevType::unknown)) != DeviceMan::SKIP_CAP.end()) {
        device = {};
        return;
    }
    ...
    if (d.type == DevType::touchpad || d.type == DevType::pointer) {
        if (j.contains(GetSettingName(SwaySetting::scroll_factor)))
            d.scroll_factor = j[GetSettingName(SwaySetting::scroll_factor)].get<float>();
    ...
    if (libinput.contains(GetSettingName(SwaySetting::tap_button_map))) {
        ...
        d.tap_button_map->select(libinput[GetSettingName(SwaySetting::tap_button_map)]);
    }
    ...
    if (libinput.contains(GetSettingName(SwaySetting::accel_speed)))
        d.accel_speed = libinput[GetSettingName(SwaySetting::accel_speed)].get<float>();
    if (libinput.contains(GetSettingName(SwaySetting::accel_profile))) {
        d.accel_profiles = SEnum({ "adaptive", "flat" });
        d.accel_profiles->select(libinput[GetSettingName(SwaySetting::accel_profile)]);
    }
    ...
    if (libinput.contains(GetSettingName(SwaySetting::click_method))) {
        ...
        d.click_methods->select(libinput[GetSettingName(SwaySetting::click_method)]);
    }
    ...
    if (libinput.contains(GetSettingName(SwaySetting::scroll_method))) {
        ...
        d.scroll_methods->select(libinput[GetSettingName(SwaySetting::scroll_method)]);
    }
    if (libinput.contains(GetSettingName(SwaySetting::scroll_button)))
        d.scroll_button = libinput[GetSettingName(SwaySetting::scroll_button)].get<int>();
...
// This function convert a value to string that works in sway config file or swaymsg calls.
template<typename T>
std::string to_string(const T& val) { return std::to_string(val); }

template<> std::string to_string(const SEnum& e) { return (std::string)e; }
template<> std::string to_string(const bool& b) { return bts(b); }
template<> std::string to_string(const CalArr& arr)     // Calibration array
{
    std::string out = "";
...
template<> std::string to_string(const std::pair<SEnum, SEnum>& pair_e) { return (std::string)pair_e.first + (std::string)pair_e.second; }
template<> std::string to_string(const std::array<int, 4>& region)   // Map to region value (maybe should be merged with CalArr above..)
{
    std::string out = "";
    for (int i = 0; i < 4; i++)
...
// Return the input parameter in format '<setting name> <setting values...>' for use in swaymsg call
inline std::string get_input_param(SwaySetting setting, const std::string& value) { return GetSettingName(setting, false) + " " + value; }
...
void opt_write(std::string swaymsg, std::string sway_id, SwaySetting setting, Opt<T, B>& value)
{
...
// Call opt_write or opt_conf depending on the is_write parameter with given arguments.
template<bool is_write, typename... Args, typename Str>
inline void opt_call(const std::string& swaymsg, const std::string& sway_id, Str&& out, Args&&... args)
{
...
// Write changes or generate config parameters into 'out' depending on the 'is_write' template parameter.
// NOTE: Why? Because I want to avoid having to type all those calls below multiple times for config generation and writes.
// NOTE: `out` is templated so that we can pass both lvalues and rvalues (when we dont use it) as parameter.
