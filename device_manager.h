...
template<typename T>
std::string GetEnumName(const std::array<std::string, (int)T::size>& enum_strings, T enum_elem) { return enum_strings[(int)enum_elem]; }

// Get enum type from name in enum_strings.
template<typename T>
Opt<T> GetEnumFromName(const std::array<std::string, (int)T::size>& enum_strings, std::string name)
{
...
/**
 * Type holding multiple string options and information about which one is selected.
 */
struct SEnum
{
    ...
    { 
        if (sel < 0 || sel >= (int)options.size())
            throw std::out_of_range("No enum is selected or corrupted index (" + std::to_string(sel) + ")");
        return options[sel]; 
    }
...
public:
    ...
    inline void RestoreBackup(int device) { ApplyChanges(device, true); m_Devices[device] = m_backupDevices[device]; }
...
inline std::string GetSettingName(SwaySetting s, bool get = true) { return GetEnumName(get ? SWAY_SETTING_GET : SWAY_SETTING_SET, s); }
inline Opt<SwaySetting> GetSetting(std::string name, bool get = true) { return GetEnumFromName<SwaySetting>(get ? SWAY_SETTING_GET : SWAY_SETTING_SET, name); }
