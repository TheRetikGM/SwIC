...
#define IMGUI_COMBO_SENUM(label, senum) ImGui::Combo(label, &senum.sel, &SEnumGetter, &senum.options, senum.options.size())

// ImGui hint on the same line as previous ImGui control.
#define IMGUI_HINT(sameline, text) if constexpr (sameline) ImGui::SameLine(); HelpMarker(text)
...
public:
    App(std::string swaymsg_path)
        : m_devMan(swaymsg_path)
    {
    ...
    static void GuiKeyboard(Device& dev)
    {
        if (dev.repeat_delay) {
            ...
            IMGUI_HINT(true, "Number of milliseconds before the key starts repeating");
        }
        if (dev.repeat_rate) {
    ...
    static void GuiLibinput(Device& dev)
    {
        ...
        if (dev.scroll_button) {
            ...
            IMGUI_HINT(true, "Sets the button used for\nscroll_method on_button_down");
        }
        if (dev.scroll_factor) {
            ImGui::SliderFloat("Scroll factor", &dev.scroll_factor.value(), 0.0f, MAX_SCROLL_FACTOR);
        ...
        if (dev.accel_profiles) {
            ...
            IMGUI_HINT(true, "adaptive - Accelerative movement\n    flat - Linear movement");
        }
    }
    ...
    void GuiRevertPopup(float dt, int selected_device, float& current_rev_timeout)
    {
    ...
    void OnUpdate(float dt)
    {
        ...
        ImGui::Combo("Device", &sel_dev, &DevGetter, &m_devMan.m_Devices, m_devMan.m_Devices.size());
        ImGui::Separator();
...
int main()
{
    ...
    def.window_hints[GLFW_RESIZABLE] = GLFW_FALSE;      // Disable resizing, so that the window is floating by default.
